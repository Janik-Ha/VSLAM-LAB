...
def extract_keypoints(args):
    ...
    for rgb_path in rgb_paths:
        ...
        img_path = os.path.join(sequence_path, rgb_path)
        ...
        r2d2_folder = os.path.join(sequence_path,'r2d2')
        
        info = np.array([W, H, len(idxs), len(desc[0])], dtype=np.float32)
        
        r2d2_folder = os.path.join(sequence_path,'r2d2')
        os.makedirs(r2d2_folder, exist_ok=True)       
        
        bin_file = os.path.join(r2d2_folder, 'info.bin')    
        with open(bin_file, 'wb') as f:
            info.tofile(f)        
            
        bin_folder = os.path.join(r2d2_folder, 'descriptors')
        os.makedirs(bin_folder, exist_ok=True)	
        bin_file = os.path.join(bin_folder, f"{img_name}.bin")
        with open(bin_file, 'wb') as f:
            desc[idxs].tofile(f)  
            
        bin_folder = os.path.join(r2d2_folder, 'keypoints')
        os.makedirs(bin_folder, exist_ok=True)	    
        bin_file = os.path.join(bin_folder, f"{img_name}.bin")
        with open(bin_file, 'wb') as f:
            xys[idxs].tofile(f)   
        
        bin_folder = os.path.join(r2d2_folder, 'scores')
        os.makedirs(bin_folder, exist_ok=True)    
        bin_file = os.path.join(bin_folder, f"{img_name}.bin")
        with open(bin_file, 'wb') as f:
            scores[idxs].tofile(f) 
...
if __name__ == '__main__':
    ...
    parser.add_argument("--sequence_path", type=str, required=True, help='path to sequence')
